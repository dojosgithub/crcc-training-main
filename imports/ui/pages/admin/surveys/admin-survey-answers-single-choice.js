/**
 * Admin Survey Answers template logic
 *
 */
import { Meteor } from 'meteor/meteor'
import { Template } from 'meteor/templating'

import { Session } from 'meteor/session'
import { ReactiveVar } from 'meteor/reactive-var'

import Sortable from 'sortablejs'

import { updateSurveyQuestionAnswer } from '/both/api/methods/training-module-survey-questions.js'
import { insertSurveyAnswer, updateSurveyAnswerStatus, updateSurveyAnswerContent, updateSurveyAnswerOrder } from '/both/api/methods/training-module-survey-answers.js'

import { TrainingModuleSurveys } from '/both/api/collections/training-module-surveys.js'
import { TrainingModuleSurveyAnswers } from '/both/api/collections/training-module-survey-answers.js'

// import '/imports/ui/stylesheets/admin/surveys/admin-survey-answers-single-choice.less'
import '/imports/ui/pages/admin/surveys/admin-survey-answers-single-choice.html'

let _selfAdminSurveyAnswersSingleChoice
let _subsAdminSurveyAnswersSingleChoice

Template.AdminSurveyAnswersSingleChoice.onCreated(function adminSurveyAnswersOnCreated() {
  _selfAdminSurveyAnswersSingleChoice = this

  _selfAdminSurveyAnswersSingleChoice.Question = this.data

  Session.set('correctSurveyAnswers', _selfAdminSurveyAnswersSingleChoice.Question.answer || [])

  _selfAdminSurveyAnswersSingleChoice.ready = new ReactiveVar()
  // _selfAdminSurveyAnswersSingleChoice.qid = new ReactiveVar(_selfAdminSurveyAnswersSingleChoice.Question._id)
  // _selfAdminSurveyAnswersSingleChoice.answer= new ReactiveVar(_selfAdminSurveyAnswersSingleChoice.Question.answer)

  _subsAdminSurveyAnswersSingleChoice = new SubsManager()

  _selfAdminSurveyAnswersSingleChoice.autorun(() => {
    // if(_selfAdminSurveyAnswersSingleChoice.qid.get()) {
      if(Session.get('curSQid')) { //-- Since this template is embedded, to reload this remotely, Session works better than local ReactiveVar.
      // let handleSurveyAnswers = _subsAdminSurveyAnswersSingleChoice.subscribe('training_module_survey_answers_w_qid', _selfAdminSurveyAnswersSingleChoice.qid.get())
      let handleSurveyAnswers = _subsAdminSurveyAnswersSingleChoice.subscribe('training_module_survey_answers_w_qid', Session.get('curSQid'))
      _selfAdminSurveyAnswersSingleChoice.ready.set(handleSurveyAnswers.ready())
    }
  })

})

Template.AdminSurveyAnswersSingleChoice.onRendered(function adminSurveyAnswersOnRendered() {
      
  //-- This not working here. See the helper 'Answers' for more details.
      let el = document.getElementById('ul_admin_survey_answers');
      let sortable = new Sortable(el, {
        // sort: true,
        onEnd: function (e) {
          e.oldIndex;  // element's old index within parent
          e.newIndex;  // element's new index within parent

          // $('#ul_admin_survey_questions').children('li.li-admin-survey-question').each(function(idx, el) { //-- not working
          $(el).children('li.li-admin-survey-answer').each(function(idx, el) {
            let aid = $(this).data('aid')
  
            let aObj = {
              _id: aid,
              order: idx
            }
            //-- Local update not working...
            // TrainingModuleQuestions.update(pid, {
            //   $set: {
            //     order: idx
            //   }
            // })

            updateSurveyAnswerOrder.call(aObj)

          })

        },
      })
  
})

Template.AdminSurveyAnswersSingleChoice.helpers({
  // Question() {
  //   let myQ =  _selfAdminSurveyAnswersSingleChoice.Question

  //   console.log(myQ)
  //   Session.set('correctSurveyAnswers', myQ.answer)

  //   return myQ
  // },
  Answers() {
    if(Session.get('curSQid')) {

        //-- Sortable is initiated only when it is here.
        //-- I guess it's because this template is embeded into 
        //-- another embedded template, and maybe needed to be 
        //-- called everytime the Dom/Answers list is created.
        // let el = document.getElementById('ul_admin_survey_answers')

        // if(el) {
        //   let sortable = new Sortable(el, {
        //     // sort: true,
        //     onEnd: function (e) {
        //       e.oldIndex;  // element's old index within parent
        //       e.newIndex;  // element's new index within parent

        //       $(el).children('li.li-admin-survey-answer').each(function(idx, el) {
        //         let aid = $(this).data('aid')
      
        //         let aObj = {
        //           _id: aid,
        //           order: idx
        //         }

        //         updateSurveyAnswerOrder.call(aObj)
        //       })
        //     },
        //   })
        // }

      //-- Then, return the main Answers cursor.
      return TrainingModuleSurveyAnswers.find({
        questionId: Session.get('curSQid')
      }, {
        sort: {
          order: 1
        }
      })
    }
  },
  // correctAnswer() {
  //   console.log(_selfAdminSurveyAnswersSingleChoice.answer.get())
  //   return _selfAdminSurveyAnswersSingleChoice.answer.get()
  // },
  correctSurveyAnswers() {    
    return Session.get('correctSurveyAnswers')
  }   
})

Template.AdminSurveyAnswersSingleChoice.events({
  'click .btn-add-new-answer'(e, tpl) {
    e.preventDefault()

    // let surveyId = _selfAdminSurveyAnswersSingleChoice.Question.surveyId
    // let questionId = _selfAdminSurveyAnswersSingleChoice.Question._id

    let surveyId = Session.get('curSvId')
    let questionId =Session.get('curSQid')

    if(surveyId !== '' && questionId !== '') {

      let answersLength = $(".ul-amdin-survey-answers").children('li.li-admin-survey-answer').length

      //-- Another reactivity issue? It dubles, so, should be divided by 2 to 
      //-- get the correct order.
      let order = answersLength /2

      let objAnswer = {
        surveyId: surveyId,
        questionId: questionId,
        order: order
      }

      TrainingModuleSurveyAnswers.insert(objAnswer, (err, res) => {
        if(err) {
          toastr.error("Something went wrong. Please try again. " + err)
        } else {
          // if(res) {
          //   let el = '<li class="li-admin-survey-answer init" data-aid="'+res+'">'
          //       el += '<input type="text" class="input-admin-survey-answer">'
          //       el += '<button class="btn btn-xs btn-default btn-delete-survey-answer">'
          //       el += '<i class="fa fa-minus"></i> </button></li>'

          //   $(".ul-amdin-survey-answers").append(el)

          // } else {
          //   toastr.error("Something went wrong. Please try again.")
          // }
        }
      })      

    }   
  },
  'click .btn-remove-this-answer'(e, tpl) {
    e.preventDefault()

    if(confirm("Are you sure to delete this answer?")) {
      let aid = $(e.currentTarget).parent().data('aid')

      let aObj = {
        _id: aid,
        status: 4
      }

      // TrainingModuleSurveyAnswers.update(aid, { //-- Not working due to SimpleSchema issue with $setOnInsert thing
      //   $set: {
      //     status: 4
      //   }      
      // }, (err, res) => {
      updateSurveyAnswerStatus.call(aObj, (err, res) => {        
        if(err) {
          toastr.error("Something went wrong. Please try again." + err)
        } else {          
        }        
      })
    }
  },
  'click .btn-save-this-answer'(e, tpl) {
    e.preventDefault()

    let aid = $(e.currentTarget).parent().data('aid')

    let content = $(e.currentTarget).siblings('.txta-survey-answer-content').val()

    if(content !== '') {

      let aObj = {
        _id: aid,
        content: content
      }

      updateSurveyAnswerContent.call(aObj, (err, res) => {        
        if(err) {
          toastr.error("Something went wrong. Please try again." + err)
        } else {
          if(res) {
            toastr.info("Successfully saved")
            $(e.currentTarget).removeClass('blink')
          } else {
            toastr.error("Something went wrong. Please try again.")
          }
        }        
      })
    }
  },
  'click .chkb-this-survey-answer'(e, tpl) {
    e.preventDefault()

    let qid = $(e.currentTarget).parent().data('qid')
    // let aid = $(e.currentTarget).parent().data('aid')

    let aids = []

    $('.chkb-this-survey-answer:checked').each(function() {
      aids.push($(this).val())
    })

    if(qid) {

      let qObj = {
        _id: qid,
        answer: aids
      }

      updateSurveyQuestionAnswer.call(qObj, (err, res) => {
        if(err) {
          toastr.error("Something went wrong. Please try again." + err)
        } else {
          if(res) {
            toastr.info("Successfully updated")
            Session.set('correctSurveyAnswers', aids)
            // _selfAdminSurveyAnswersSingleChoice.answer.set(aids)
          } else {
            toastr.error("Something went wrong. Please try again.")
          }
        }        
      })
    }
  },
  'click .radio-this-survey-answer'(e, tpl) {
    e.preventDefault()

    let qid = $(e.currentTarget).parent().data('qid')
    // let aid = $(e.currentTarget).parent().data('aid')

    let aids = []
    
    aids.push($('.radio-this-survey-answer:checked').val())    

    if(qid) {

      let qObj = {
        _id: qid,
        answer: aids
      }

      updateSurveyQuestionAnswer.call(qObj, (err, res) => {
        if(err) {
          toastr.error("Something went wrong. Please try again." + err)
        } else {
          if(res) {
            toastr.info("Successfully updated")
            Session.set('correctSurveyAnswers', aids)
            // _selfAdminSurveyAnswersSingleChoice.answer.set(aids)
          } else {
            toastr.error("Something went wrong. Please try again.")
          }
        }        
      })
    }
  },  
  'click .btn-update-answer-status'(e, tpl) {
    e.preventDefault()

    let aid = $(e.currentTarget).parent().data('aid')
    let status = $(e.currentTarget).data('status')

    if(aid) {
      let aObj = {
        _id: aid,
        status: status
      }

      updateSurveyAnswerStatus.call(aObj, (err, res) => {
        if(err) {
          toastr.error("Something went wrong. Please try again." + err)
        } else {
          if(res) {
            toastr.info("Successfully updated")            
          } else {
            toastr.error("Something went wrong. Please try again.")
          }
        }        
      })

    }


  }
  // 'blur .txta-survey-answer-content'(e, tpl) {
  //   e.preventDefault()

  //   $(e.currentTarget).siblings('.btn-save-this-answer').addClass('blink')
  // }
})

Template.AdminSurveyAnswersSingleChoice.onDestroyed(() => {
  Session.set('correctSurveyAnswers', null)
  _subsAdminSurveyAnswersSingleChoice.clear()
})


