...
let _selfAdminSurveyAnswers
let _subsAdminSurveyAnswers

Template.AdminSurveyAnswers.onCreated(function adminSurveyAnswersOnCreated() {
  _selfAdminSurveyAnswers = this

  _selfAdminSurveyAnswers.Question = this.data.Question

  Session.set('correctSurveyAnswers', _selfAdminSurveyAnswers.Question.answer)

  _selfAdminSurveyAnswers.ready = new ReactiveVar()
  // _selfAdminSurveyAnswers.qid = new ReactiveVar(_selfAdminSurveyAnswers.Question._id)
  // _selfAdminSurveyAnswers.answer= new ReactiveVar(_selfAdminSurveyAnswers.Question.answer)

  _subsAdminSurveyAnswers = new SubsManager()

  _selfAdminSurveyAnswers.autorun(() => {
    // if(_selfAdminSurveyAnswers.qid.get()) {
      ...
      // let handleSurveyAnswers = _subsAdminSurveyAnswers.subscribe('training_module_answers_w_qid', _selfAdminSurveyAnswers.qid.get())
      let handleSurveyAnswers = _subsAdminSurveyAnswers.subscribe('training_module_answers_w_qid', Session.get('curSQid'))
      _selfAdminSurveyAnswers.ready.set(handleSurveyAnswers.ready())
    }
  })

})

Template.AdminSurveyAnswers.onRendered(function adminSurveyAnswersOnRendered() {
...
Template.AdminSurveyAnswers.helpers({
  answerType() {
  ...
  // Question() {
  //   let myQ =  _selfAdminSurveyAnswers.Question
  ...
  // correctAnswer() {
  //   console.log(_selfAdminSurveyAnswers.answer.get())
  //   return _selfAdminSurveyAnswers.answer.get()
  // },
...
Template.AdminSurveyAnswers.events({
  'click .btn-add-new-answer'(e, tpl) {
    e.preventDefault()

    // let surveyId = _selfAdminSurveyAnswers.Question.surveyId
    // let questionId = _selfAdminSurveyAnswers.Question._id
      ...
      updateQuestionAnswer.call(qObj, (err, res) => {
        if(err) {
          ...
          if(res) {
            ...
            // _selfAdminSurveyAnswers.answer.set(aids)
          } else {
            toastr.error("Something went wrong. Please try again.")
          }
        }        
...
Template.AdminSurveyAnswers.onDestroyed(() => {
  Session.set('correctSurveyAnswers', null)
  _subsAdminSurveyAnswers.clear()
})
